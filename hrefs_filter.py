...
def find_live_match_links_all(driver, allowed_categories=None, allowed_surfaces=None):
    ...
    for block in tournament_blocks:
        try:
            tournament_type_element = block.find_element(By.CSS_SELECTOR, ".wcl-overline_rOFfd.wcl-scores-overline-05_P-fjE.wclLeagueHeader__overline.wclLeagueHeader__textColor")
            tournament_type = tournament_type_element.text.strip()

            tournament_full_name_element = block.find_element(By.CSS_SELECTOR, ".event__titleBox > a.wcl-link_bLtj3.wcl-linkBase_CdaEq.wcl-primary_C2HA0.wclLeagueHeader__textColor")
            tournament_full_name = tournament_full_name_element.text.strip()

            if tournament_type in allowed_categories and "Qualification" not in tournament_type:
                found_surface = None
                ...
                if found_surface:
                    ...
                    for child in parent_element.find_elements(By.XPATH, "./*"):
                        ...
                        if found_tournament_block and child.tag_name == 'div':
                            if "event__match" in child.get_attribute("class") and "event__match--live" in child.get_attribute("class"):
                                try:
                            ...
                            elif "wcl-header_uBhYi" in child.get_attribute("class") and "wclLeagueHeader--collapsed" in child.get_attribute("class"):
                                print("  Встречен следующий блок турнира. Завершаем поиск матчей для текущего турнира.")
                                break # Встретили следующий блок турнира, прекращаем поиск матчей для текущего
                ...
                print(f"Турнир '{tournament_full_name}' не соответствует фильтрам по типу или содержит 'Qualification'.")
